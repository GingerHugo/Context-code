...
static void info(const char *fmt,...) {}
#endif
void null(const char *s) {}
extern "C" {extern double dnrm2_(int *, double *, int *);}
...
{
	double best_rate;
	cross_validation_search_C(prob,param,nr_fold, param->C, param->C, 2, target, &best_rate);
}

void find_parameter_C(const problem *prob, const parameter *param, int nr_fold)
{
	...
	if(param->solver_type == L2R_LR)
		min_C = 1.0 / (prob->l * max_norm);
	else if(param->solver_type == L2R_L2LOSS_SVC)
		min_C = 1.0 / (2 * prob->l * max_norm);

	start_C = pow( 2, floor(log(min_C)/log(2)) );

	best_C = cross_validation_search_C(prob, param, nr_fold, start_C, 1024, 2, target, &best_rate);
	if(best_C > 0)
		printf("Best C = %lf  CV accuracy = %g%%\n", best_C, 100.0*best_rate);
	free(target);
}

double cross_validation_search_C(const problem *prob,const parameter *param, int nr_fold, double start_C, double max_C, double ratio, double *target, double* best_rate)
{
	...
	int change_count=0;
	struct model **submodel = Malloc(model*,nr_fold);
	...
	while(param1.C <= max_C)
	{
		...
		for(i=0; i<nr_fold; i++)
		{
			...
			struct model *tmp_model = submodel[i];
			submodel[i] = warm_start_train(&subprob[i],&param1,submodel[i]);

			if(tmp_model != NULL)
			{
				int nr_model = 1;
				if(submodel[i]->nr_class > 2)
					nr_model = submodel[i]->nr_class;
				int dim = subprob[i].n * nr_model ;
				int inc = 1;
				for(j=0; j<dim; j++)
					tmp_model->w[j] = tmp_model->w[j] - submodel[i]->w[j];

				double dwnorm = dnrm2_(&dim, tmp_model->w,&inc);
				if(dwnorm > 1e-15)
					change_count=0;

				free_and_destroy_model(&tmp_model);
			}

			for(j=begin; j<end; j++)
				target[perm[j]] = predict(submodel[i],prob->x[perm[j]]);
		}
		...
		if(total_correct > best_correct)
		{
			...
			best_C = param1.C;
			*best_rate = (double)best_correct / prob->l;
		}

		
		info("log2c=%7.2f\trate=%g\n",log(param1.C)/log(2),100.0*total_correct/prob->l);
		if(change_count > 2)
			break;
	...
	for(i=0;i<nr_fold;i++)
		free_and_destroy_model(&submodel[i]);
	free(fold_start);
	free(perm);
